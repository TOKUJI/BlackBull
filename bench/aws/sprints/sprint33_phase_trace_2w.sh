#!/usr/bin/env bash
# Sprint 33 — 1-worker vs 2-worker phase trace on the same EC2 box.
#
# Same workload as sprint33_phase_trace_ec2.sh (TLS HTTP/1.1, .br
# precompressed), sweeps {1 worker, 2 workers} × {c=64, c=1024} on a
# single c7i.2xlarge so the inter-worker scaling story is apples-to-
# apples (same kernel, same ENA queue assignment, same wheel).
#
# Goal: rule out / confirm per-worker linear scaling — if 2-worker
# r/s ≈ 2 × 1-worker r/s, BlackBull scales fine and the 32-worker
# HttpArena cliff is elsewhere.  If 2-worker r/s grows sub-linearly,
# there's a per-worker contention cost that compounds.
#
# Cost: c7i.2xlarge ~ $0.36/hr × ~20 min ≈ $0.13.
...
LOCAL_DEST="$REPO_ROOT/bench/results/aws/sprint33-phase-trace-2w-${TS}"
mkdir -p "$LOCAL_DEST"

echo "=== sprint33_phase_trace_2w.sh ==="
...
# Sweep (workers, conns).  Same instance, fresh server process per cell.
...
# EC2 1-vs-2 worker phase trace — Sprint 33
...
- wrk:      t=$THREADS d=${DURATION_S}s; sweep WORKERS={1,2} × CONNS={64,1024}
EOF
