"""Every configurable environment variable: its default, and what it does.

One module, so that three things stated separately cannot disagree -- which
variables exist, what they fall back to, and what each one means.
``blackbull.env.get_settings`` reads the defaults from here, and
``scripts/gen_env_docs.py`` writes the tables in ``docs/reference/env-vars.md``
from the docstrings below.  A description is wrong in one place, or in none.

Private because publishing it would render every description twice: once on
this module's own reference page and once on the page generated from it.

Each docstring is the Markdown of one table cell, on one line, unwrapped so
that a round trip through the generator changes nothing.  Section headings and
order below are the page's.
"""
from __future__ import annotations

import os

#: Two per core, at least four: one compression job can saturate a core, so
#: the ceiling is the machine rather than a number someone picked.
...
BB_WORKERS = 1
"""Pre-fork worker count.  `0` resolves to `os.cpu_count()`.  Each worker runs its own asyncio event loop; combine with `BB_SOCKET_REUSEPORT=1` so the kernel load-balances accepts across workers — except under `--reload`, where the master keeps one listener for its exec and the workers share it, and on a port-bound non-ASGI protocol, which has one owner.  See [Workers](../deployment/workers.md)."""
...
"""Dual-path conformance lane.  When set, every request round-trips the native `Connection` through `as_scope()` + `from_scope()` before dispatch, so the ASGI compatibility conversion is exercised on the self-hosted path and cannot silently bitrot.  Enabled in CI; leave it off in normal operation, where the native path skips the extra round-trip."""

BB_CPU_PINNING = 'auto'
"""Per-worker CPU pinning, applied after fork.  `auto` gives worker *i* the *i*-th CPU of the mask the process already has, so `taskset` / `numactl` / a cpuset are honoured rather than overridden; `off` pins nothing; an explicit `taskset`-style list (`2,4,6-9`; `0` is CPU 0, not the off switch) confines workers to those CPUs, intersected with the mask granted to the process.  Only the event loop is pinned — the thread pool behind `run_in_executor` compression and `asyncio.to_thread` file reads keeps the full mask, since offloading exists to get off the loop's core.  Multi-worker and Linux only; a single-worker server is never pinned.  Set `off` on a shared or externally-orchestrated host."""
...
"""`TCP_USER_TIMEOUT` socket option (Linux).  Per-connection upper bound on how long an unacknowledged sent segment can linger before the kernel kills the connection.  Useful to evict dead peers behind NATs without waiting for keepalives.  Set on the listening sockets BlackBull builds itself — a single-worker bind, the master's socket that workers share, a port-bound non-ASGI protocol, and the listener a `--reload` master keeps — and inherited by the TCP connections accepted from them, which is where it takes effect.  No other listener carries it: an fd adopted from a supervisor keeps the value its creator set, the per-worker listeners `BB_SOCKET_REUSEPORT=1` binds with more than one worker are built without it, and an `AF_UNIX` socket has no such option.  See "Performance recommendations" below for production tuning.  `0` leaves the kernel default in place."""
...
"""`listen()` backlog depth.  A sane default for servers facing connection bursts (128 — the traditional `SOMAXCONN` — is shallow next to nginx's 511).  Linux caps the effective value at `net.core.somaxconn`.  See "Performance recommendations" below for production tuning.  Accepting does not start until lifespan startup completes, so connections arriving before then queue in the kernel.  On a listener BlackBull binds itself, **this value bounds that queue** — per listening socket, and a dual-stack listener has two of them, so `BB_SOCKET_REUSEPORT=1` with N workers binds N such pairs; the kernel admits one connection past it.  An fd adopted from a supervisor is already listening, so its queue answers to the backlog its creator passed to `listen()` until accepting starts, which re-applies this value to that same socket — except on the multi-worker `BB_SOCKET_REUSEPORT=1` path without `--reload`, which discards the adopted fd and binds fresh listeners that carry this value from the start.  See [Unix sockets and fd inheritance](../deployment/unix-and-fd.md#fd-inheritance-systemd-socket-activation).  `BB_MAX_CONNECTIONS` is an accept-time decision and does not bound the window, whose length is the startup hook's own runtime and which the framework puts no deadline on.  A TCP arrival past this bound is not refused but delayed: the kernel drops the SYN and the client retransmits, so it is admitted once accepts begin.  That delay is budgeted by the client's own SYN retransmission (about two minutes on a stock Linux), not by this value — a window longer than that budget **loses** the excess instead of delaying it, the client's kernel giving up on a connection that was never refused.  Each parked connection is held by the kernel rather than by the process, up to one socket receive buffer (128 KiB on a stock Linux, and nothing reads during the window, so it does not autotune upward; `BB_SOCKET_RCVBUF` moves that number on the listeners its own entry reaches); on each listener BlackBull binds, at the default of 1024 that is about 128 MiB of kernel memory for as long as the window lasts, invisible to the process's own descriptor count and memory."""

BB_SOCKET_REUSEPORT = False
"""When supported by the OS (Linux, modern BSDs), bind each worker to its own listening socket so the kernel hashes incoming connections across workers — eliminates the thundering-herd accept pattern.  It is the dual-stack TCP bind that is repeated per worker: an `AF_UNIX` listener never is (with this on, `workers > 1` beside a `unix_path` fails to start unless `--reload` is set too: the per-worker re-bind takes the port off the socket it replaces, and an `AF_UNIX` name is a path, not a port), and a listener adopted from a supervisor is used as-is with one worker and under `--reload`, and closed and re-bound per worker only with more than one worker and no `--reload`.  Because a supervisor such as systemd keeps the socket, that re-bind fails for every address the creator still holds: the family it holds is left unserved — the whole port when that socket is dual-stack — while connections queue on the creator's socket unanswered.  With one worker the socket carries the option and has no second queue to hash across; under `--reload` one master-owned listener is kept to hand across the exec and nothing is spread.  Enable on multi-worker deployments; a **pessimization under connection churn** (short-lived connections), where the hash spreads bursts unevenly — see [Workers](../deployment/workers.md#workers-vs-cores-under-connection-churn).  `0` leaves the kernel default in place."""

BB_SOCKET_SNDBUF = 0
"""`SO_SNDBUF` (bytes) on every listening socket BlackBull builds — a single-worker bind, the master's socket that workers share, a port-bound non-ASGI protocol, and the listener a `--reload` master keeps — and, on TCP, on the connections accepted from those sockets, which inherit it.  It does not reach an fd adopted from a supervisor, which keeps the send buffer its creator set; the per-worker listeners `BB_SOCKET_REUSEPORT=1` binds with more than one worker are built without it; and an `AF_UNIX` listener takes it while its accepted connections carry the kernel's own default.  `0` leaves the kernel default unchanged.  Linux clamps the request at `net.core.wmem_max` and then doubles it; larger values help throughput for responses ≥ 64 kB."""

BB_SOCKET_RCVBUF = 0
"""`SO_RCVBUF` (bytes) in the same scope as `BB_SOCKET_SNDBUF` — the same listeners and, on TCP, the connections accepted from them — against `net.core.rmem_max` instead, and doubled by the same rule.  `0` leaves the kernel default unchanged."""
...
"""Seconds a `StaticFiles` cache entry trusts its last `stat()`.  Keeps edit-on-disk visibility under a second while taking the per-request `stat` off the cache-hit path.  `0` stats on every request."""


# --- HTTP/2 internals ----------------------------------------------------------

BB_H2_INITIAL_WINDOW_SIZE = 65535
"""Per-stream flow-control window advertised in the server's initial `SETTINGS` frame.  Larger lets peers send more data per stream before waiting for `WINDOW_UPDATE`.  See "Performance recommendations" below.  The default is RFC 7540 §6.9.2's."""

BB_H2_CONNECTION_WINDOW_SIZE = 65535
"""Connection-level flow-control window advertised via an initial `WINDOW_UPDATE` on stream 0.  Must be ≥ 65535; smaller values are silently ignored.  See "Performance recommendations" below.  The default is RFC 7540 §6.9.2's minimum."""
...
"""How many bytes of a server-streaming response are gathered before a write.  Batching amortises the per-write cost; no message is ever withheld past the call, because the batch is flushed before any trailing status."""
...
"""Maximum concurrent compression offloads to the asyncio default thread pool.  When at or above this cap, additional eligible responses are served **uncompressed** rather than queued — bounded fall-back rather than unbounded queue growth.  The floor of 4 keeps a one- or two-CPU host overlapping a few offloads instead of serialising them.  `0` removes the cap, leaving an unbounded executor queue that saturates under burst load."""

BB_BROTLI_QUALITY = 4
"""Brotli quality level (0–11) for dynamic-response compression.  4 matches Google/Cloudflare's recommendation for dynamic content; 5 matches Apache `mod_brotli`; 6 matches nginx `ngx_brotli`.  11 is appropriate only for build-time / static pre-compression — far too expensive on the request path."""


# --- Diagnostic timing ---------------------------------------------------------

BB_PHASE_TRACE = '0'
"""Record per-request wall-clock and CPU checkpoints into the access-log record's `phases`.  Off by default because the extra `perf_counter()` and `process_time()` calls are visible in benchmark numbers; intended for one-off investigation, not production."""
