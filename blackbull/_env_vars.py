...
BB_UVLOOP = False
"""Install `uvloop`'s asyncio policy at startup.  Requires `pip install 'blackbull[speed]'`; falls back to the standard loop with a warning when uvloop is missing.  Under uvloop the connection cap does not pause accepting; size it by [Sizing the connection cap under uvloop](../deployment/unix-and-fd.md#sizing-the-connection-cap-under-uvloop)."""
...
"""On Linux and modern BSDs, give each worker its own listening socket so the kernel hashes connections across workers, removing the thundering herd — but a **pessimization under connection churn**, where the hash spreads a burst unevenly.  Only the TCP bind is repeated per worker, at the address the master bound: an `AF_UNIX` listener never is, so `workers > 1` beside a `unix_path` fails to start unless `--reload` is set too.  `0` keeps the kernel default.  See [Per-path scope](../deployment/workers.md#socket-options-across-bind-paths).  An adopted fd (`--bind fd://N`) is split this way only while the socket is the workers' to re-bind: a supervisor that keeps it makes startup refuse, naming this setting, the worker count and the listener — see [fd inheritance](../deployment/unix-and-fd.md#several-workers-behind-an-activated-socket)."""
