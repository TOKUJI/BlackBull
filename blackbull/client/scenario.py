...
in order against a live socket.  The same data model is consumed by:

  * the differential test
    (:mod:`tests.conformance.http1.test_http1_differential`),
    which generates scenarios via Hypothesis and compares BlackBull's
    response to nginx's.
  * the atheris fuzz harness
    (:mod:`tests.conformance.http1.fuzz.fuzz_http1`), which decodes
    coverage-guided mutated bytes into scenarios via
    :meth:`Scenario.from_bytes`.

Two serialisations are supported:

  * :meth:`Scenario.to_json` / :meth:`Scenario.from_json` — JSON Lines,
    one step per line, for the on-disk corpus under
    ``tests/conformance/http1/fuzz/corpus/*.jsonl``.  Diff-friendly in
    git; readable when failures are pasted into reports.
