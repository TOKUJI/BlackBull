...
| `BB_CLIENT_MIN_BODY_RATE` | `240.0` | Minimum sustained response-body delivery rate in **bytes per second**; below it, past `BB_CLIENT_MIN_BODY_RATE_GRACE`, the response is abandoned with a `TimeoutError`.  `BB_CLIENT_BODY_TIMEOUT` returns on *any* arrival, so it degrades to "send something every 30 s", which a one-byte drip always satisfies — a rate is what a drip cannot fake.  Averaged over a rolling window one grace period wide, so a burst that ran ahead buys that window and not the whole response.  **On by default**, unlike `BB_CLIENT_BODY_MAX_TOTAL`: a total refuses a response for being large and legitimate responses are large, while 240 B/s is below 2G, below satellite, below anything a real transfer sustains.  Matches `BB_MIN_BODY_RATE`.  `0` disables. |
| `BB_CLIENT_MIN_BODY_RATE_GRACE` | `5.0` | Seconds of body delivery before `BB_CLIENT_MIN_BODY_RATE` is enforced — the slow-start allowance — and the width of the window the average is taken over.  What is measured is **transport-wait time only**, so an application slow to consume is never mistaken for a peer slow to send. |
