...
<!-- generated: Runtime and processes -->
...
<!-- generated: Connection limits and timeouts -->
| Variable | Default | Controls |
|---|---|---|
...
| `BB_REQUEST_TIMEOUT` | `0.0` | Per-HTTP/2-stream deadline in seconds.  When the deadline elapses the stream is forcibly cancelled with `RST_STREAM CANCEL`.  Use a positive value (e.g. `30`) in production to evict stalled handlers from stream slots.  Off by default. |
...
| `BB_WRITE_TIMEOUT` | `30.0` | Seconds the server will wait for a single response socket drain, `sendfile` chunk, or HTTP/2 flow-control wait.  This is per progress wait, not a whole response total; `0` disables. |
...
Four knobs are shared in that sense.  The rest are named for the framing they
belong to — HTTP/2 has no field *line* to cap, for instance, so what bounds a
field there is `BB_CLIENT_H2_MAX_HEADER_LIST_SIZE` over the whole section.
Each entry above states its own scope.

What differs among the four is the unit each is denominated in, because the
framings do:
...
<!-- generated: HTTP/2 internals -->
| Variable | Default | Controls |
|---|---|---|
| `BB_H2_INITIAL_WINDOW_SIZE` | `65535` | Per-stream flow-control window advertised in the server's initial `SETTINGS` frame.  Larger lets peers send more data per stream before waiting for `WINDOW_UPDATE`.  See "Performance recommendations" below.  The default is RFC 7540 §6.9.2's. |
| `BB_H2_CONNECTION_WINDOW_SIZE` | `65535` | Connection-level flow-control window advertised via an initial `WINDOW_UPDATE` on stream 0.  Must be ≥ 65535; smaller values are silently ignored.  See "Performance recommendations" below.  The default is RFC 7540 §6.9.2's minimum. |
...
| `BB_MQTT_BROKER_INBOX_MAXSIZE` | `1024` | Positive bound on messages waiting in the worker's MQTT broker inbox. Readers await admission at capacity; the broker logs the cap hit. Independent of session QoS backlog. Invalid values, including zero, use the default. |
| `BB_MQTT_BROKER_INBOX_MAX_BYTES` | `16777216` (16 MiB) | Positive wire-size budget for broker inbox contents, alongside the count cap. Compact ACK/lifecycle envelopes charge one byte and one slot. A single packet exceeding the budget is refused with `DISCONNECT 0x97`. Not a heap budget; active processing and each reader's admission candidate are outside the queue. Invalid values use the default. |
| `BB_MQTT_CONNECTION_INBOX_MAXSIZE` | `1024` | Positive bound on all packets waiting for each MQTT writer, including QoS 0 and ACKs. The producer yields to the writer at capacity, then logs and ends only that connection if it cannot admit the packet; it never waits for socket progress inside the broker. Invalid values, including zero, use the default. |
| `BB_MQTT_CONNECTION_INBOX_MAX_BYTES` | `16777216` (16 MiB) | Positive encoded-byte budget for each MQTT writer inbox. The packet object is retained alongside encoded bytes. One active write is outside this queue budget and uses `BB_WRITE_TIMEOUT`. Tune with the accepted packet size; an individually larger packet cannot fit even in an empty queue. Invalid values use the default. |
...
| `BB_GRPC_STREAM_BATCH_BYTES` | `16384` | How many bytes of a server-streaming response are gathered before a write.  Batching amortises the per-write cost; no message is ever withheld past the call, because the batch is flushed before any trailing status. |
...
| `BB_COMPRESSION_EXECUTOR_THRESHOLD` | `65536` | Body size above which compression is offloaded to a thread-pool executor so the event loop stays responsive during the (CPU-bound) compress call.  `0` always compresses on the event loop. |
| `BB_COMPRESSION_MAX_INFLIGHT` | `max(cpu_count() * 2, 4)` | Maximum concurrent compression offloads to the asyncio default thread pool.  When at or above this cap, additional eligible responses are served **uncompressed** rather than queued — bounded fall-back rather than unbounded queue growth.  The floor of 4 keeps a one- or two-CPU host overlapping a few offloads instead of serialising them.  `0` removes the cap, leaving an unbounded executor queue that saturates under burst load. |
...
| `BB_SOCKET_SNDBUF` | `0` | `262144` | 256 KiB requested.  The kernel accepts at most `net.core.wmem_max` of it and then doubles that, so the effective size is `2 × min(262144, wmem_max)` — 512 KiB where the sysctl allows it, 416 KiB at the stock `212992`, where bumping `net.core.wmem_max` is what buys the rest.  Helps throughput on responses ≥ 64 kB (static assets, JSON arrays, streamed bodies). |
| `BB_SOCKET_RCVBUF` | `0` | `262144` | Same shape against `net.core.rmem_max`, for inbound traffic (large `POST` bodies). |
...
`BB_SOCKET_REUSEPORT=1` with more than one worker makes the other three
socket-option rows inert **on the HTTP listeners it repeats per worker**:
those binds carry `BB_SOCKET_BACKLOG` and the option itself but not
`BB_SOCKET_SNDBUF`, `BB_SOCKET_RCVBUF` or `BB_TCP_USER_TIMEOUT_MS`.  A
port-bound non-ASGI protocol is not repeated — it keeps the one listener the
master built for it, which carries all of them.
[Socket tuning](#socket-tuning) above states the scope of each.
