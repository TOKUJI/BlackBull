...
**Two and a half knobs are not shared yet**, and until they are, a peer
that negotiates `h2` does escape them:

| knob | on HTTP/2 |
|---|---|
| `BB_CLIENT_HEAD_TIMEOUT` | **half shared.**  It bounds the wait for END_HEADERS once a field block is open — the peer has committed to a message and stopped mid-delivery — running from when the block opened rather than from the last frame, so a stream of zero-length CONTINUATION frames cannot re-arm it forever.  It does **not** bound the wait for the *first* response frame: a peer that completes the handshake and then says nothing still parks the call. |
...
What differs among the three that *are* shared is the unit each is
...
stream and the connection — and every other stream on it — survives.
