...
**Three knobs are not shared yet**, and until they are, a peer that
negotiates `h2` does escape them:

| knob | on HTTP/2 |
|---|---|
| `BB_CLIENT_HEAD_TIMEOUT` | nothing bounds the wait for the *first* response frame; a peer that completes the handshake and then says nothing parks the call. |
