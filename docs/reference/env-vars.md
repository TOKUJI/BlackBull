...
| `BB_WS_QUEUE_DEPTH` | `0` | WebSocket inbound **read-ahead** depth.  `0` (default) reads frames inline, in the handler's own task, when it calls `receive()` — no reader task and no per-message queue hop.  A positive value restores a background reader that reads *ahead* of the handler into a queue of that depth, so control frames are serviced between `receive()` calls and up to `N` messages buffer under a slow handler.  Control frames are serviced even at depth `0` when the handler isn't reading: at the next `send()` and on every deadline-scanner tick for a connection idle more than ~0.3 s (the idle watchdog), bounding PONG latency to ~one tick with no per-connection timers.  Registering a `websocket_message` listener does **not** force read-ahead on — a consuming handler keeps the inline win; a deferred reader starts only after the handler goes quiet (see [WebSocket guide](guide.md) → "Read-ahead and back-pressure"). |
