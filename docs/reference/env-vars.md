...
| `BB_BODY_CHUNK_SIZE` | `65536` | Slice size (bytes) for streaming an HTTP/1.1 request body to the ASGI app, on **both** framings.  The body is delivered as successive `http.request` events (`more_body: True` until exhausted) instead of one allocation the size of the declared length — capping per-connection buffering at O(chunk × connections).  It bounds a `Content-Length` body by the remaining count and a `Transfer-Encoding: chunked` body by what is left of the chunk in progress, so a peer-declared `chunk-size` never sets the read size.  64 KiB matches asyncio's default `StreamReader` buffer and sits below the backpressure high-water mark, which is what lets the pause work.  Must be `> 0` (invalid values fall back to the default). |
| `BB_BODY_CHUNK_MAX` | `524288` | Ceiling (bytes) for the adaptive `Content-Length` body read.  `BB_BODY_CHUNK_SIZE` is only the *starting* slice: while a peer keeps the transport running ahead of the server the slice doubles, up to this ceiling, so a large upload costs fewer receive round-trips; two consecutive reads that drain the transport halve it again, never below `BB_BODY_CHUNK_SIZE`.  Raise it for large uploads over fast links.  The ceiling is a **latency** bound rather than a memory one — reads are exact-size, so a slice is also a promise to deliver that many bytes inside `BB_BODY_TIMEOUT`, and a slow client on a large slice can run out of time.  Set equal to `BB_BODY_CHUNK_SIZE` for fixed-size slices.  Values below it are raised to it.  The sizing rule is adapted from Netty's [`AdaptiveRecvByteBufAllocator`](https://netty.io/4.1/api/io/netty/channel/AdaptiveRecvByteBufAllocator.html). |
